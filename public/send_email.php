...
// Валидация данных
if (empty($data['name']) || empty($data['phone']) || empty($data['email']) || empty($data['service'])) {
...
try {
    ...
    // Тема письма
    $mail->Subject = 'Новая заявка на консультацию конференц-зала';
    ...
    $mail->AltBody = "Новая заявка на консультацию\n\nИмя: " . $data['name'] . "\nТелефон: " . $data['phone'] . "\nEmail: " . $data['email'] . "\nУслуга: " . $data['service'] . "\nДата: " . date('d.m.Y H:i:s');
